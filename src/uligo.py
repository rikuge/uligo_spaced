...
##   This is the main part of uliGo 0.4, a program for practicing
##   go problems. For more information, see http://www.u-go.net/uligo/

##   Copyright (C) 2001-12 Ulrich Goertz (u@g0ertz.de)
##   Internationalization support was added by Sergey Astanin.
...
#import webbrowserIOError as io
...
class BunchTkVar:
    ...
    def loadFromDisk(self, filename, onFailure = lambda:None):
        try:
            ...
            for x in list(self.__dict__.keys()):
                if x in d: self.__dict__[x].set(d[x])
...
class EnhancedCursor(Cursor):
    ...
    def next(self, varnum=0):
        ...
        if not self.wrongVariation and ('TR' in n or 'WV' in n): self.wrongVariation = 1
...
class StatusBar(Frame):
    ...
    def set(self, status=None):
        if   status == 'solved1':
            self.label.config(fg='DarkGreen', text=_('solved!'), font='24')
        elif status == 'solved2':
            self.label.config(fg='blue', text=_('solved'), font='24')
        elif status == 'wrong':
            self.label.config(fg='red', text=_('wrong!'), font='24')
        elif status == 'end':
            self.label.config(fg='red', text=_('end'), font='24')
        else:
...
class PbmRecord:
    ...
    def getNext(self):
        ...
        if self.modeVar.get() == 0:
            if randint(0,1):
                self.handleCurrent = randint(0,int(self.noOfPbms//3))
            else:
                self.handleCurrent = randint(0,int(self.noOfPbms//2))
    ...
    def saveToDisk(self):
        ...
        string_write = 'uligo03\n'
        string_write = str.encode(string_write)
        f.write(string_write)


        help = self.modeVar.get()
        #HIER NOG GEKKE DINGEN?
...
class App:
    ...
    def convCoord(self, x, orientation=0):
        ...
        m = list(zip('abcdefghijklmnopqrs', range(1,20)))
        pos0 = list(filter(lambda z, x=x: z[0]==x[0], m))[0][1]
        pos1 = list(filter(lambda z, x=x: z[0]==x[1], m))[0][1]
    ...
    def setup(self, c, n = '', changeOri=1, co=None, ori=None):
        ...
        pbmName = ''
        if 'GN' in c.currentNode(): pbmName = c.currentNode()['GN'][0][:15]
        if n: pbmName = pbmName + n
        self.pbmName.set(pbmName)

        # look for first relevant node
        while not ('AB' in c.currentNode() or 'AW' in c.currentNode() \
                   or 'B' in c.currentNode() or 'W' in c.currentNode()): c.next()

        while not ('AB' in c.currentNode() or 'AW' in c.currentNode() or 'B' in c.currentNode() or 'W' in c.currentNode()): c.next()

        # and put stones on the board
        while not ('B' in c.currentNode() or 'W' in c.currentNode()):
            if 'AB' in c.currentNode():
                for x in c.currentNode()['AB']:
                    self.board.play(self.convCoord(x, self.orientation), bColor)
            if 'AW' in c.currentNode():
                for x in c.currentNode()['AW']:
                    self.board.play(self.convCoord(x, self.orientation), wColor)
            c.next()

        if 'B' in c.currentNode():   self.board.currentColor = self.inputColor = bColor
        elif 'W' in c.currentNode(): self.board.currentColor = self.inputColor = wColor
        c.previous()
        self.cursor = c
    ...
    def nextMove(self, p):
        ...
        try:
            ...
            for i in range(self.cursor.noChildren()):
                ...
                if nM in c and self.convCoord(c[nM][0], self.orientation)==(x,y): # found the move
                    done = 1
        ...
        try:
            ...
            for i in range(self.cursor.noChildren()):             # look for the move in the SGF file
                if (not done):
                    ...
                    if nM in c and self.convCoord(c[nM][0], self.orientation)==p:  # found
                        self.board.play(p)
                        self.board.delMarks()
                        done = 1
    ...
    def readProblemColl(self, path = None, filename = None):
        ...
        if not path or not filename:
            path, filename = os.path.split(tkinter.filedialog.askopenfilename(filetypes=[('SGF files', '*.sgf'), ('All files', '*')], initialdir=self.sgfpath))
    ...
    def nextProblem(self):
        ...
        try:
            ...
            showwarning(_('Parsing Error'), _('Error in SGF file 1!'))
            return
    ...
    def setWhoseTurn(self, c):
        ...
        self.whoseTurnCanv.create_text(x-d//3,y+3*d//2, text=turnmsg)
    ...
    def backProblem(self):
        if self.pbms:
            ...
            try:
                ...
                showwarning(_('Parsing Error'), _('Error in SGF file 3!'))
                return
            ...
            try:
                ...
                showwarning(_('Parsing Error'), _('Error in SGF file 4!'))
                return
    ...
    def restartProblem(self):
        ...
        try:
            ...
            showwarning(_('Parsing Error'), _('Error in SGF file 5!'))
            return
    ...
    def replayGame(self, path=None, filename=None, force=0):
        ...
        try:
            ...
            showwarning(_('Parsing Error'), _('Error in SGF file 6!'))
            return
        ...
        try:
            ...
            for tag, label in [('PW', 'White: '), ('PB', 'Black: '), ('DT', 'Date: '), ('RE', 'Result: ')]:
                if tag in d: self.gameInfoCanv.create_text(10, 20+20*i, text=label+d[tag][0], anchor='w')
                i += 1
        except SGFError:
            showwarning(_('SGF Error'), _('Error in SGF file!'))
            return
    ...
    def nextMoveReplay(self, p):
        ...
        try:
            ...
            if nM in c: right_pos = self.convCoord(c[nM][0])
            if nM in c and self.convCoord(c[nM][0], self.orientation)==(x,y): # found the move
                done = 1
                self.guessSuccess()
    ...
    def guessSuccess(self):
        ...
        self.guessModeCanvas.create_text(110, 20, text = repr(self.guessRightWrong[0]) + '/' + \
                                         repr(self.guessRightWrong[0] + self.guessRightWrong[1]), tags='non-bg')
        self.guessModeCanvas.create_text(110, 40, text = repr(perc) + '%', tags='non-bg')
    ...
    def helpAbout(self):
        ...
        t = 'uliGo 0.4 - written by Ulrich Goertz (u@g0ertz.de)\n\n'
    ...
    def helpDoc(self):
        ...
        webbrowser.open('file:'+os.path.abspath(os.path.join(sys.path[0], '../doc/_build/html', 'index.html')), new=1)
    ...
    def __init__(self, master):
        ...
        for b, im in [ (self.backButton, 'go-previous'), (self.restartButton, 'reload_page'),
                       (self.nextButton, 'go-next'), (self.hintButton, 'stock_help'),
                       (self.showSolutionButton, 'dialog-information'), (self.undoButton, 'edit-undo'),
                       (self.tryVarButton, 'system-search') ]:
            try:
                self.tkImages.append(PhotoImage(file = os.path.join(self.uligopath, 'gifs', im+'.gif')))
                b.configure(image=self.tkImages[-1])
            except ImportError:
                pass
        ...
        self.backButton.pack(side=LEFT, expand=YES, fill=BOTH, padx=5, pady=5)
        self.restartButton.pack(side=LEFT, expand=YES, fill=BOTH, padx=5, pady=5)
        self.nextButton.pack(side=LEFT, expand=YES, fill=BOTH, padx=5, pady=5)
        self.hintButton.pack(side=LEFT, expand=YES, fill=BOTH, padx=5, pady=5)
        self.showSolutionButton.pack(expand=YES, fill=BOTH, padx=5, pady=5)
        self.undoButton.pack(side=LEFT, expand=YES, fill=BOTH, padx=5, pady=5)
        self.tryVarButton.pack(expand=YES, fill=BOTH, padx=5, pady=5)
        ...
        try:
            ...
            # FIXME this does not work if the board is too small
