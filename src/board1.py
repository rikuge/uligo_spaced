...
##   This file is part of uliGo, a go database program
...
class abstractBoard:
    ...
    def play(self,pos,color):
        ...
        if pos in self.status:               # check if empty
            return 0
    ...
    def legal(self, pos, color):
        ...
        for x in self.neighbors(pos):
            if x in self.status and self.status[x]==self.invert(color):
        ...
        while foundNew:
            ...
            for x in newlyFound:
                for y in self.neighbors(x):
                    if y not in self.status and y != exc:    # found a liberty
                        return []
                    elif y in self.status and self.status[y]==self.status[x] \
...
class Board(abstractBoard, Canvas):
    ...
    def drawBoard(self):
        ...
        if self.img:
            self.delete('board')
            size_adj = int(size / 100)
            for i in range(size_adj + 1):
                for j in range(size_adj + 1):
        ...
        if self.coordinates:
            for i in range(self.boardSize):
                ...
                self.create_text(c0//4+1, c0+c1*i, text=repr(self.boardSize-i),font = ('Helvetica', 5+c1//7, 'bold'))
                self.create_text(c1*self.boardSize+3*c0//4+4, c0 + c1*i, text=repr(self.boardSize-i), font = ('Helvetica', 5+c1//7, 'bold'))
    ...
    def resize(self, event = None):
        ...
        for x in list(self.status.keys()): self.placeStone(x, self.status[x])
        for x in list(self.marks.keys()): self.placeMark(x, self.marks[x])
        for x in list(self.labels.keys()): self.placeLabel(x, '+'+self.labels[x][0], self.labels[x][1])
    ...
    def placeLabel(self, pos, type, text=None, color=None, override=None):
        ...
        if override:
            fcolor = override[0]
            fcolor2 = override[1]
        elif pos in self.status and self.status[pos]=='B':
            fcolor = 'white'
            fcolor2 = '#FFBA59'
        elif pos in self.status and self.status[pos]=='W':
            fcolor = 'black'
            fcolor2 = ''
        else:
    ...
    def clear(self):
        ...
        for x in list(self.stones.keys()):
            self.delete(self.stones[x])
        self.stones = {}
        self.onChange()
    ...
    def fuzzyStones(self):
        ...
        for p in list(self.status.keys()):
